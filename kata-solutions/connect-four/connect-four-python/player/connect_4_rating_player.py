...
class ConnectFourRatingPlayer(ComputerPlayer):
  ...
  def rate_move(self, move, player = Disc.YELLOW, depth = 0):
    ...
    if self.board.has_connect_four():
      ...
      rating = depth if player == Disc.RED else -depth 
      ...
      for move in range(1, self.board.get_col_count() + 1):
        if self.board.is_valid_move(move):
          ratings[move - 1] = self.rate_move(move, player, depth + 1)

      rating = max(ratings) if player == Disc.YELLOW else min(ratings)
  ...
  def calculate_next_move(self):
    ...
    for move in range(1, self.board.get_col_count() + 1):
      ratings[move -1] = self.rate_move(move) \
        if self.board.is_valid_move(move) else -1000000
