...
class FileStatistics:
    def __init__(self, lines):
        ...
        self._numbers = []
        self._words = []
        [self._process_single_line(line) for line in lines]
    ...
    def word_frequencies(self):
        ...
        for word in self.lower_case_words():
            word_counts[word] = 1 if word not in word_counts.keys() else word_counts[word] + 1
        return word_counts
    
    def word_frequencies_max(self):
        word_counts = self.word_frequencies()
        return 0 if len(word_counts.items()) == 0 else reduce(max, word_counts.values())
    
    def most_common_words(self):
        most_common_words = []
        for item in self.word_frequencies().items():
            count = item[1]
    ...
    def max_value(self):
        return None if len(self._numbers) ==0 \
            else reduce(max, self._numbers)
...
class ReportGenerator:
    def get_report(self, stats):
        most_common_str = "".join(word + ", " for word in stats.most_common_words())
