...
from src.poker_hands import determine_best_hand
from src.illegal_args_exception import IllegalArgumentsException
import src.poker_hands
...
with description("Ranking hands") as self:
  ...
  with context("Given a straight flush hand"):
    with before.each:
      ...
      when(src.poker_hands).rank_hand(self.straight_flush).thenReturn(PokerRanks.STRAIGHT_FLUSH.value)
    ...
    with context("and a full house hand"):
      with before.each:
        ...
        when(src.poker_hands).rank_hand(self.full_house).thenReturn(PokerRanks.FULL_HOUSE.value)
